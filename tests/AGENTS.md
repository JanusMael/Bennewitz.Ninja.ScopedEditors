...
|---|---|
| `Architecture/` | `LayeringTests`, `AssemblyQualityTests` (AQ1001–AQ1004 over every shipped assembly), `PackageMetadataTests`, and the markup guards `ThemeResourceIntegrityTests`, `DangerSurfaceMarkupTests`, `SeverityGlyphFontSizeMarkupTests` |
...
| Every scan asserts it inspected something before asserting it found nothing | Zero findings from a scan that saw nothing reads as a clean codebase | `AssemblyQualityTests.Every_shipped_assembly_is_in_the_scan`; `PackageSources.Markup`; the `Inspected` assertions in each markup guard |
...
| `LayeringTests.Tiers` is the single table of what each project may reach | `AssemblyQualityTests.AQ1003_no_assembly_references_what_its_tier_forbids` reads it too; a copy would rot | `LayeringTests.Every_tier_named_here_actually_exists` |
