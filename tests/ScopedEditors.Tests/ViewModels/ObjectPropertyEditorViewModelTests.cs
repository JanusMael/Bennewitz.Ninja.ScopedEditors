...
public class ObjectPropertyEditorViewModelTests
{
    ...
    public void Children_AreExposed_FromConstructor()
    {
        ...
        Assert.Same(boolChild, vm.Children[0]);
    }
    ...
    public void LoadFromValue_WithWorkspace_LoadsChildren()
    {
        ...
        Assert.True(boolChild.Value);
        Assert.Equal("loaded", stringChild.Value);
    }
    ...
    public void ResetToInherited_ClearsAllChildren()
    {
        ...
        Assert.Null(boolChild.Value);
        Assert.Null(stringChild.Value);
    }
    ...
    public void ChildPropertyChanged_Bubbles_SecondModification_ForceFires()
    {
        ...
        Assert.True(vm.IsModified);
        MessageAssert.Equal(1, raisedAfter, "Force-fire required: hosting group editor must re-read ToValue() "
    ...
    public void ChildPropertyChanged_Ignores_NonIsModifiedNotifications()
    {
        ...
        Assert.True(stringChild.IsModified);
        MessageAssert.Equal(1, raised, "The IsModified bubble fires once; non-IsModified property changes "
    ...
    public void ChildPropertyChanged_LastChildClearsModified_FiresFalse()
    {
        ...
        Assert.False(vm.IsModified);
        MessageAssert.Equal(1, raisedAfter, "Clearing the last modified child fires IsModified=false on the parent.");
    }
    ...
    public void LoadFromValue_PropagatesEffectiveScopeAndIsOverridden()
    {
        ...
        Assert.NotNull(vm.EffectiveScope);
        Assert.True(vm.IsOverridden, "Two scopes have values → IsOverridden=true should propagate.");
    }
    ...
    public void NonClaudeConsumer_CanDriveLibrary_EndToEnd()
    {
        ...
        Assert.Null(enabledChild.Value); // User has no value; Project has it
        Assert.Equal("user-label", labelChild.Value);
        ...
        Assert.True(enabledChild.Value);

        // Act: reset child
        enabledChild.ResetToInheritedCommand.Execute(null);
        Assert.Null(enabledChild.Value);
        Assert.False(enabledChild.IsModified);
    }
    ...
    public void ToValue_PreservesKeysNoChildModels_AtEditingScope()
    {
        ...
        MessageAssert.Equal(1, vm.Children.Count, "premise: exactly one modelled child");
        ...
        Assert.NotNull(result);
        Assert.False((bool?)result["flag"], "the edit itself must still land");
        Assert.Equal("http://proxy.internal:8080", result["HTTPS_PROXY"]);
        Assert.Equal("/opt/acme/bin", result["INTERNAL_TOOL_PATH"]);
    }
    ...
    public void ToValue_RoundTripsUnmodelledKeys_WithNoEditAtAll()
    {
        ...
        Assert.NotNull(result);
        MessageAssert.Equal(3, result.Count, "load-then-save dropped keys the editor chose not to render");
    }
    ...
    public void ToValue_DoesNotCarryUnmodelledKeys_FromAnotherScope()
    {
        ...
        Assert.NotNull(result);
        Assert.False(result.ContainsKey("HTTPS_PROXY"), "a key defined only at PROJECT must not be re-emitted at USER");
    }
    ...
    public void ResetToInherited_DropsPreservedKeys()
    {
        ...
        MessageAssert.Null(vm.ToValue(), "reset left an object alive on keys the user cannot see");
    }
    ...
    public void ToValue_ChildValueWins_OverAPreservedKeyOfTheSameName()
    {
        ...
        Assert.NotNull(result);
        Assert.Single(result);
        Assert.False((bool?)result["flag"], "the child's current value must win");
    }
}
