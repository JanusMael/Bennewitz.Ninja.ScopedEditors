...
public sealed class PropertyEditorAssessDangerTests
{
    ...
    public void OwnPathReturnsOwnAssessment()
    {
        ...
        MessageAssert.NotNull(assessment, "An editor must answer for its own path.");
        Assert.Equal("share held 'unsafe'", assessment.Explanation);
        Assert.True(assessment.IsDangerNow, "The editor's own current value must decide IsDangerNow.");
    }
    ...
    public void ForeignPathReturnsNullRatherThanUnremarkable()
    {
        ...
        MessageAssert.Null(editor.AssessDanger("snapshot"), "A path this editor does not render must come back null, NOT Unremarkable. Both render "
    ...
    public void PathMatchIsOrdinalNotCaseInsensitive()
    {
        ...
        MessageAssert.Null(editor.AssessDanger("Permission"), "These are JSON paths, not display text. A case-insensitive match would let one key "
    ...
    public void AnEditorWithNoClassifierStillOwnsItsPath()
    {
        ...
        MessageAssert.NotNull(assessment, "Ownership is about which editor renders the path, not whether the product triaged it. "
            + "Returning null here would make a product with no danger table indistinguishable "
            + "from a path no editor renders.");
        Assert.Equal(DangerAssessment.Unremarkable, assessment);
    }
    ...
    public void NestedPathIsAnsweredByTheChildThatRendersIt()
    {
        ...
        MessageAssert.NotNull(assessment, "The descent must find a child's path through IChildEditorHost.");
        MessageAssert.Equal("attachment.image.maxWidth held 'unsafe'", assessment.Explanation, "The CHILD's assessment must come back — carrying the child's own value. Answering "
    ...
    public void DescentReachesAChildEditorHostThatIsNotTheLibrarysOwnObjectEditor()
    {
        ...
        MessageAssert.NotNull(outer.AssessDanger("a.b.c"), "The descent must be driven by the IChildEditorHost interface, not by a concrete "
    ...
    public void ForeignPathIsNullEvenWhenChildrenExist()
    {
        ...
        MessageAssert.Null(parent.AssessDanger("attachment.image.maxHeight"), "A miss below a parent that HAS children must still be null, or the walk stops early.");
    }
}
