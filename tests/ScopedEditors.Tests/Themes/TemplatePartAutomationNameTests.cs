...
namespace Bennewitz.Ninja.ScopedEditors.AvaloniaUI.Tests.Themes;
...
public sealed class TemplatePartAutomationNameTests
{
    ...
    public Task NumericUpDown_FocusTarget_InheritsTheNameTheViewSet()
    {
        return Session.Dispatch(() =>
        {
            ...
            Assert.True(subject.Text.Focusable, "The inner PART_TextBox is not focusable, so this test is no longer about " +
                "the element that takes focus. Re-measure where focus lands before trusting it.");

            MessageAssert.Equal(SpinnerUnderTest.HostName, PeerName(subject.Text), $"A NumericUpDown named '{SpinnerUnderTest.HostName}' has an inner PART_TextBox " +
    ...
    public Task AutoCompleteBox_FocusTarget_InheritsTheNameTheViewSet()
    {
        return Session.Dispatch(() =>
        {
            ...
            MessageAssert.Equal(AutoCompleteUnderTest.HostName, PeerName(subject.Text), $"An AutoCompleteBox named '{AutoCompleteUnderTest.HostName}' has an inner " +
    ...
    public Task FocusTarget_OfAnUnnamedHost_StaysUnnamed()
    {
        return Session.Dispatch(() =>
        {
            ...
            Assert.True(string.IsNullOrEmpty(PeerName(subject.Text)), $"A NumericUpDown with no AutomationProperties.Name produced an inner text box " +
    ...
    {
        MessageAssert.NotEqual(WrapperStrings.LabelSpinnerIncrease, WrapperStrings.LabelSpinnerDecrease, "Both spinner directions resolve to the same text, so a screen-reader user " +
    ...
    public Task Expander_HeaderPart_WithAComplexHeader_InheritsTheNameTheViewSet()
    {
        return Session.Dispatch(() =>
        {
            ...
            MessageAssert.Equal(ExpanderUnderTest.HostName, PeerName(subject.Header), $"An Expander named '{ExpanderUnderTest.HostName}' has an ExpanderHeader part " +
    ...
    public Task Expander_HeaderPart_WithAStringHeader_AlsoInheritsTheHostsName()
    {
        return Session.Dispatch(() =>
        {
            ...
            MessageAssert.Equal(ExpanderUnderTest.HostName, PeerName(subject.Header), $"An Expander with Header=\"{ExpanderUnderTest.StringHeaderText}\" named " +
    ...
    public Task Expander_HeaderPart_OfAnUnnamedHost_IsNotGivenAnInventedName()
    {
        return Session.Dispatch(() =>
        {
            ...
            MessageAssert.NotEqual(ExpanderUnderTest.HostName, PeerName(subject.Header), "An unnamed Expander's header part picked up another Expander's name, which means " +
                "the binding is resolving to the wrong ancestor.");

            Assert.True(string.IsNullOrEmpty(PeerName(subject.Header))
                || PeerName(subject.Header)!.StartsWith("Avalonia.", StringComparison.Ordinal), $"An unnamed Expander's header part announces '{PeerName(subject.Header)}'. That " +
    ...
    public Task Expander_HeaderPart_OfANamedHost_NeverAnnouncesAFrameworkTypeName()
    {
        return Session.Dispatch(() =>
        {
            ...
            foreach (ExpanderUnderTest.HeaderKind kind in named)
            {
                ...
                Assert.False(announced is not null && announced.StartsWith("Avalonia.", StringComparison.Ordinal), $"With header kind {kind} the Expander's header part announces '{announced}' — " +
    ...
    private static void AssertAnnounces(RepeatButton button, string expected, string direction)
    {
        ...
        Assert.False(string.IsNullOrWhiteSpace(announced), $"The {direction} button of a NumericUpDown announces nothing. Themes/" +
        ...
        MessageAssert.NotEqual(contentTypeName, announced, $"The {direction} button of a NumericUpDown announces '{announced}' — the " +
        ...
        Assert.False(announced!.StartsWith("Avalonia.", StringComparison.Ordinal), $"The {direction} button of a NumericUpDown announces '{announced}', which is a " +
        ...
        MessageAssert.Equal(expected, announced, $"The {direction} button announces '{announced}' rather than the " +
    ...
    private sealed class SpinnerUnderTest : IDisposable
    {
        ...
        internal SpinnerUnderTest(bool nameTheHost = true)
        {
            ...
            try
            {
                List<ButtonSpinner> spinners = numeric.GetVisualDescendants()
                                                      ...
                                                      .ToList();
                MessageAssert.Equal(1, spinners.Count, $"Expected a templated NumericUpDown to contain exactly one ButtonSpinner, " +
        ...
        private static RepeatButton FindRepeatButton(ButtonSpinner spinner, string partName)
        {
            ...
            MessageAssert.Equal(1, matches.Count, $"Expected exactly one RepeatButton named '{partName}' in the ButtonSpinner " +
    ...
    private sealed class ExpanderUnderTest : IDisposable
    {
        ...
        private static ToggleButton FindHeaderPart(Expander host)
        {
            ...
            MessageAssert.Equal(1, matches.Count, $"Expected exactly one ToggleButton named 'ExpanderHeader' templated by the " +
    ...
    private static TextBox FindTextBoxPart(Control host)
    {
        ...
        MessageAssert.Equal(1, matches.Count, $"Expected exactly one TextBox named 'PART_TextBox' templated by " +
