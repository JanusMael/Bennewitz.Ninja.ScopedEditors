...
public sealed class TreeChevronAutomationNameTests
{
    ...
    public Task TheChevronAnnouncesItsName() => Session.Dispatch(() =>
    {
        ...
        MessageAssert.Equal(WrapperStrings.LabelExpandCollapse, name, $"the expand/collapse chevron announces '{name}'. A screen reader reaches one of these "
    ...
    public Task ThePremiseHolds_TheChevronIsTheOnlyExpandCollapseAffordance() => Session.Dispatch(() =>
    {
        ...
        MessageAssert.Null(itemPeer.GetProvider<IExpandCollapseProvider>(), "the TreeViewItem now exposes an ExpandCollapse pattern. That changes the F6 decision: "
        ...
        MessageAssert.NotNull(chevronPeer.GetProvider<IToggleProvider>(), "the chevron no longer exposes a Toggle pattern, so nothing in the tree offers "
    ...
    private sealed class TreeUnderTest : IDisposable
    {
        ...
        internal TreeUnderTest()
        {
            ...
            try
            {
                ...
                MessageAssert.Equal(1, parts.Count, $"expected exactly one {ChevronPart} under a templated TreeViewItem with "
                ...
                MessageAssert.IsAssignableFrom<ToggleButton>(parts[0], $"{ChevronPart} is a {parts[0].GetType().Name}, not a ToggleButton. The style "
                    + "selector names ToggleButton, so it now matches nothing and the name is "
                    + "silently absent.");

                MessageAssert.Same(Item, parts[0].TemplatedParent, $"{ChevronPart}'s TemplatedParent is "
