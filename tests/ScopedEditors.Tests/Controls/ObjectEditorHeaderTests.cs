...
public sealed class ObjectEditorHeaderTests
{
    ...
    private static string WrapperPath()
    {
        ...
        for (int i = 0; i < 12 && !string.IsNullOrEmpty(dir); i++)
        {
            string candidate = Path.Combine(
                dir, "src", "ScopedEditors.AvaloniaUI", "Controls", "PropertyEditorWrapper.axaml");
    ...
    public void ObjectExpanderHeader_DoesNotBindDisplayName()
    {
        ...
        MessageAssert.NotNull(header, "The object template's expander should still declare a header.");
        Assert.False(header.Contains("DisplayName", StringComparison.Ordinal), "The expander header binds DisplayName, which the wrapper's label row already shows — "
    ...
    public void TheLabelRow_StillShowsTheName()
    {
        ...
        Assert.True(labelled, "PropertyNameLabel no longer binds DisplayName. The duplication must be fixed by "
