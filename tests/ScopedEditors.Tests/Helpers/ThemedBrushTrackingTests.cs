...
namespace Bennewitz.Ninja.ScopedEditors.AvaloniaUI.Tests.Helpers;
...
public sealed class ThemedBrushTrackingTests
{
    ...
    public Task AThemedBrushFollowsTheVariantAfterItHasBeenHandedOut()
    {
        return Session.Dispatch(() =>
        {
            ...
            try
            {
                ...
                MessageAssert.IsAssignableFrom<ISolidColorBrush>(result, "Expected a solid colour brush from the themed dictionary. If this fails the "
                    + "key was not found and every assertion below is measuring the fallback.");

                ISolidColorBrush handedOut = (ISolidColorBrush)result;
                MessageAssert.Equal(LightValue, handedOut.Color, "Light variant colour");

                // Flip the variant WITHOUT re-running the converter, which is exactly the
                // situation an already-rendered row is in.
                app.RequestedThemeVariant = ThemeVariant.Dark;

                MessageAssert.Equal(DarkValue, handedOut.Color, "The brush an element is already holding did not follow the theme. It is a "
    ...
    private static void BrushHelper_ResetForTesting()
    {
        ...
        Type? helper = typeof(AppSeverityToBrushConverter).Assembly
            .GetType("Bennewitz.Ninja.ScopedEditors.AvaloniaUI.Helpers.BrushHelper");
