...
public sealed class AppSeverityToFontSizeConverterTests
{
    ...
    {
        Assert.True(MeasuredInkAt14[AppSeverity.Critical] < MeasuredInkAt14[AppSeverity.Caution], "the measured ink table no longer says Critical draws smaller than Caution, so the "
            + "defect this converter exists to fix is not the defect these numbers describe");

        Assert.True(MeasuredInkAt14[AppSeverity.Critical] < MeasuredInkAt14[AppSeverity.Neutral], "the measured ink table no longer says the hollow Neutral circle out-draws Critical. "
    ...
    public void TheScalesBeatTheMeasuredInkDisparity()
    {
        ...
        {
            Assert.True(critical > DrawnInk(quieter), $"Critical draws {critical:F2} of ink against {quieter}'s {DrawnInk(quieter):F2}. "
        ...
        Assert.True(DrawnInk(AppSeverity.Caution) > DrawnInk(AppSeverity.Info), "Caution must out-draw Info, or the amber tier reads as quieter than the blue one");
    }
    ...
    public void TheCirclesAreDeliberatelyUnscaled()
    {
        ...
        {
            MessageAssert.Equal(1.0, AppSeverityToFontSizeConverter.ScaleFor(circle), 1e-9,
    ...
    public void EverySeverityGetsAFiniteScaleAboveZero()
    {
        foreach (AppSeverity s in Enum.GetValues<AppSeverity>())
        {
            ...
            Assert.True(double.IsFinite(scale) && scale > 0, $"{s} scales by {scale}, which would render its glyph invisible or throw");
        }
    }
    ...
    public void TheTiersStayDistinct()
    {
        foreach (AppSeverity s in Enum.GetValues<AppSeverity>())
        {
            Assert.True(AppSeverityToFontSizeConverter.SizeFor(s, 14d)
                > AppSeverityToFontSizeConverter.SizeFor(s, 11d), $"{s} renders the same size on the 14pt and 11pt tiers");
        }
    }
    ...
    {
        MessageAssert.NotEqual((double) Convert(AppSeverity.Critical, "11"), (double) Convert(AppSeverity.Critical, "14"), "the ConverterParameter changes nothing, so the tier base is not being read and both "
            + "tiers would render at one size");
    }

    [Theory]
    [InlineData(null, TestDisplayName = "no parameter")]
    [InlineData("", TestDisplayName = "empty")]
    [InlineData("fourteen", TestDisplayName = "not a number")]
    [InlineData("0", TestDisplayName = "zero")]
    [InlineData("-11", TestDisplayName = "negative")]
    [InlineData("NaN", TestDisplayName = "NaN")]
    ...
    {
        Assert.Equal(AppSeverityToFontSizeConverter.SizeFor(AppSeverity.Neutral, 14d), (double) Convert("not a severity", "14"), 1e-9);

        Assert.Equal(AppSeverityToFontSizeConverter.SizeFor(AppSeverity.Neutral, 14d), (double) Convert(null, "14"), 1e-9);
    }
