...
|---|---|---|
| The Avalonia package's id is `Bennewitz.Ninja.ScopedEditors.Avalonia`; its assembly and namespaces are `ScopedEditors.AvaloniaUI` | A namespace segment named `Avalonia` shadows Avalonia's own root namespace (CS0234); renaming the id instead strands every consumer on a dead id | `PackageId` and `AssemblyName` in `src/ScopedEditors.AvaloniaUI/ScopedEditors.AvaloniaUI.csproj`; `AssemblyQualityTests.AQ1004_no_namespace_segment_shadows_a_referenced_root`; `PackagingTests.Every_packable_project_is_classified` |
